...
**阶段二：经济模型与账户体系 (我们的下一个目标)**

*   **目标**: 引入账户（钱包）和账本（UTXO集）的概念，让区块链能够真正地管理“价值”。
*   **关键任务**:
    1.  **`createwallet` 命令**: 实现钱包功能，能够创建新的用户地址。
    2.  **UTXO 集重构**: 创建一个高效的 UTXO 数据库索引。每当新区块被添加时，都需要同步更新这个索引（花掉旧的 UTXO，产生新的 UTXO）。
    3.  **`getbalance --address <ADDRESS>` 命令**: 实现查询余额功能。这个命令将直接查询 UTXO 集，而不是每次都遍历整个区块链，从而获得极高的性能。
